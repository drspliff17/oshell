...
media_clear_player :: proc(media: ^Media_State) {
	if media.player_len > 0 do media.player[0] = 0
	media.player_len = 0
}

media_set_player :: proc(media: ^Media_State, player: string) {
	media.player_len = min(len(player), len(media.player) - 1)
	if media.player_len > 0 do copy(media.player[:media.player_len], player[:media.player_len])
...
media_clear_artist :: proc(media: ^Media_State) {media.artist_len = 0}

media_set_artist :: proc(media: ^Media_State, artist: string) {
	media.artist_len = min(len(artist), len(media.artist))
	if media.artist_len > 0 do copy(media.artist[:media.artist_len], artist[:media.artist_len])
}
...
// Title
media_clear_title :: proc(media: ^Media_State) {media.title_len = 0}

media_set_title :: proc(media: ^Media_State, title: string) {
	media.title_len = min(len(title), len(media.title))
	if media.title_len > 0 do copy(media.title[:media.title_len], title[:media.title_len])
}
...
media_has_player :: proc(media: ^Media_State) -> bool {return media.player_len > 0}

media_visible :: proc(media: ^Media_State) -> bool {return media.playing && media.title_len > 0}
