...
setup_signals :: proc() {
	...
	usr2_handler := posix.signal(.SIGUSR2, handle_sigusr2)
	if rawptr(usr2_handler) == posix.SIG_ERR {
		fmt.eprintln("Failed to install SIGUSR2 handler")
		return
	}
	...
	if posix.siginterrupt(.SIGUSR2, true) != .OK do fmt.eprintln("Failed to configure SIGUSR1 interrupt behaviour")
